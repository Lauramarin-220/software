/**
 * pantalla  principal del panel de administrador y auxiliar 
 ...
 * incluye acceso rapidos y las secciones mas usados 
 * muestra infromacion del sistema (estado de la api y rol del usuario)
 */
...
/**
 * control de rol
 * solo necesita rol y nombre de usuarios en pantalla 
 */
type AuthUser = { rol?: string; nombre?: string };

/**
 * panel de navegacion
 * el tipo de router de expo no expone el push con tipado correcto 
 * las versiones actuales del ios y android no permiten use de push 
 * 
 */
...
// tipo statCard
// Describe la forma de cata tarjeta de estadistica de grid
...
type StatCard = {
    ...
    gradient: [ string, string ]; // par de colores de fondo PRINCIPAL y fondo secundario
    route: string; // ruta a la que navega al presionar la tarjeta 
    show: boolean; // si es falso la tarjeta no se muestra
};
// componente principal del dashboard
export default function AdminDashboardScreen() {
    // contexto de autenticacion
    // se usa cast a tipo explicito porque AuthCntext.js es js puro y no lo reconoce TSX
    ...
    const isAux = user?.rol === 'auxiliar'; // true solo para auxliares 

    /**
     * estad local 
     * objeto con todos los contadores que se muestran en grip de tarejtas 
    ...
    const [ stats, setStast ] = useState ({
        ...
        subcategorias: 0,
        productos: 0,
        usuarios: 0, // numero de usuarios registrados solo de admin
        pedidos: 0,
        pendientes: 0,
        ventas: 0
    });
    ...
    // efecto de carga de estadisticas 
    useEffect(() => {
        const load = async () => {
            setLoading(true);
            try {
                // se hace 4 peticiones en paralelo con promise.all para reducir el timepo de carga 
                ...
                const [ cats, subs, prods, orders ] = await Promise.all([
                    ...
                    apiClient.get('/admin/productos?limite=1'), // solo porque necesita el total
                ...
                // los stast quedan en 0 no se muestran mensaje de error al usuario
                //ignore 
            } finally {
                setLoading(false);
            }
        ...
        /**
         * solo carga estadisticas si el usuario esta autenticado y tiene el rol administrador o auxiliar 
         */
        if (isAuthenticated && (isAdmin || isAux)) load();

    }, [ isAuthenticated, isAdmin, isAux ]); // se ejecuta segun el rol de inicio sesion
    ...
    if (!isAuthenticated || (!isAdmin && !isAux)) {
        ...
        <View style={styles.centered}>
            ...
            color="#ccc" />
            <Text style={styles.restrictedTitle}> Acesso restringido</Text>
            <Text style={styles.restrictedSub}> Acesso restringido</Text>
        </View>
        )
    }
