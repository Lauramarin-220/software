/**
 * Este archivo o pantalla es la lista de pedidos en panel de administrador
 ...
 * solo para ro de admin y auxiliar 
 */
...
export default function AdminPedidoScreen() {
    ...
    //estado local 
    const [ pedidos, setPedidos ] = useState<Pedido[]>([]); //esto trae unas Array de pedidos de la pagina actual
    ...
    const fetchPedidos = async (page = 1, search = '') => {
        ...
        try {
            ...
            const params: string[] = [];
            if (search.trim()) params.push(`buscar=${encodeURIComponent(search.trim())}`); // codifica los caracteres especiales en la busqueda 
            params.push(`pagina=${page}`); // numero de la pagina 
            params.push('limite=10'); // 10 pedidos por pagina 
            const url = `/admin/pedidos?${params.join('&')}` //construye l URL completa con los parametros 
            ...
            setPagina
            (page); //actualiza la pagina actual
            setTotalPaginas(res.data?.data?.totalPaginas || 1);
        } catch (error: unknown) {
     ...
     * avanza o retrocede la pagina actual next +1 para siguiente  -1 para anterior 
