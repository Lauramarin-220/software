...
import { ActivityIndicator, Alert, Dimensions, FlatList, Modal, Image, Pressable, ScrollView, RefreshControl, StyleSheet, TextInput, View } from "react-native";
...
export default function HomeScreen() {
    ...
    const handleAgregarAlCarrito = async (producto: any) => {
        try {
            ...
            Alert.alert('Error', msg || 'No fue posible agregar el producto al carrito')
        }
    };

    /**
     * funcion loadCatologo
     * cargar los productos y categorias desde el backend
     * en paralelo con promise.all
     * isRefresh: true cuando es un pull tu refresh  
     */

    const loadCatalogo = async ({ isRefresh = false } = {}) => {
        if (isRefresh) setRefreshing(true); // pull-to-refresh activa indicador de refresco
        else setLoading(true); // carga inicial activa el spiner grande 
        setErrorMessage('');
        try {
            /**
             * llama a todos los eniponts al msmo tiempo para cargar velocidad 
             * getProductos con limite de 200 trae todos los productos una sola vez
             */
            const [productosData, categoriasData] = await Promise.all([ 
            catalogoService.getProductos({ pagina: 1, limite: 200}),
            catalogoService.getCategorias(),
            ]);
            // guardar los datos solo si son arrays
     ...
     * como dependecias se ejecuta una sola vez al montar el componente
     */
    ...
    /**
     * efecto resetear pagina al cambiar filtros cuando
     * el usuario escribe en el buscador o ambia
     * de categoria
     * siempr
     */
     ...
     * useMemo recalcula solo cuando cambian las dependecias internas 
     * evitando filtrar/calcular en cada renderizado 
    */

    // Productos filtrados subcojunto de productos que coinciden con la busqueda y categorias 
    const productosFiltrados = useMemo(() => {
        const termino = busqueda.trim().toLowerCase(); // se normaliza el texto
        return productos.filter(( p: any ) => {
            //coincidenTexto el producto tiene el termino en su nombre descripcion
            const coincideTexto = 
            termino === '' ||
            //sin busqueda para toda la informacion de los productos
            p.nombre?.toLowerCase().includes(termino) ||
            p.descripcion?.toLowerCase().includes(termino);

            //coincideCategorias el producto pertenece a la categoria activa 
            //categoriaId o categotia.id son los dos campos posibles segun el endpoint
            const coincideCategorias =
            categoriaActiva === 'all' ||
            //pasa toso
            String(p.categoriaId || p.categoria?.id) === categoriaActiva;
            return coincideTexto && coincideCategorias; 
        })
    }, [busqueda, categoriaActiva, productos]); // re calcula cuando cambia la busqueda categoria o datos 

    /**
     * hasproductos: true si hay al menos un producto 
     * filtrado evita mostrar la vista vacia 
     */
    const hasProductos =  useMemo (() => productosFiltrados.length > 0, [productosFiltrados]);

    // total paginas numero total de paginas segun los products filtrados 

    const totalPaginas  =  useMemo (() => Math.ceil(productosFiltrados.length / ITEMS_POR_PAGINA), [productosFiltrados, ITEMS_POR_PAGINA]);

    // productos visibles subcojunto de productos de la pagina actual eje pagina 1, 1-15 pagina 2 16-30...
...
// ── SUBCOMPONENTE: ListHeader ────────────────────────────────────────────
  ...
  const renderProducto = ({ item: producto, index }: { item: any; index: number }) => (
    <View
      ...
      <Image
        source={{ uri: catalogoService.buildImagenUrl(producto.imagen) }}
          ...
          <ThemedView style={styles.modalCard}>
            ...
            {productoDetalle ? (
              <>
                ...
                <Image
                  source={{ uri: catalogoService.buildImagenUrl(productoDetalle.imagen) }}
